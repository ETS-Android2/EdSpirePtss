...
public class AddButtonQue extends Fragment {
    ...
    String selectedDept, editPostId,isUpdateKey;
    Context context;
    ...
    public void updateQuestion(final String selectedSubject, final String selectedDept, String editPostId) {
        ...
        UsersRef.child(currentUserID).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    HashMap userMap = new HashMap();
                    userMap.put("uid", currentUserID);
                    userMap.put("Postkey", editPostId);
                    userMap.put("username", username);
                    userMap.put("profileImage", userprofileImg);
                    userMap.put("question", userQuestion);
                    userMap.put("date", saveCurrentDate);
                    userMap.put("time", saveCurrentTime);
                    UsersReference.child(editPostId).updateChildren(userMap).addOnCompleteListener(new OnCompleteListener() {
                        @Override
                        public void onComplete(@NonNull Task task) {
                            if (task.isSuccessful()) {
                                Toast.makeText(context, "Post sucessfully updated", Toast.LENGTH_LONG).show();
                                return;
                            } else {
                                Toast.makeText(context, task.getException().toString(), Toast.LENGTH_LONG).show();
                                return;
